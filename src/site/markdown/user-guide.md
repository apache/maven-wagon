...
constant `Wagon.ROLE` — and role hint equal to the URL scheme. None is a singleton, so
every lookup gives you a fresh instance; a Wagon is not safe to share between
concurrent sessions.

Since 4.0.0 the providers are JSR-330 `@Named` beans, indexed into
`META-INF/sisu/javax.inject.Named`, so the container has to be told to read that index:
...
Maven itself already configures its container this way, so builds are unaffected.

A container built with `new DefaultPlexusContainer()` has classpath scanning off and
reads only `META-INF/plexus/components.xml`. That still finds `wagon-http` and
`wagon-webdav-jackrabbit`, which keep hand-written descriptors because they map one
class onto several hints, but no other provider — a partial failure that is easy to
misread. If a lookup that used to work now fails, this is why.

Put the provider artifact on the classpath and, with index scanning on, its components
